...
import type { LoginResponse, UserLoginForm } from "../../../../tracepulse-fe/src/types"
import { useMutation } from "@tanstack/react-query"
import { toast } from "sonner"
import { useAuth } from "../../../../tracepulse-fe/src/contexts/AuthContext"
import { UserService } from "../../../../tracepulse-fe/src/api/generated"
// Si usas lucide-react (común en Vite), puedes importar estos iconos:
...
export default function LoginView() {
    ...
    const { mutate } = useMutation({
        mutationFn: (credentials: UserLoginForm) => UserService.userAuthLoginCreate(credentials),
        onError: (error: any) => {
    ...
    const handleLogin = (formState: UserLoginForm) => {
        mutate(formState)
    }
    ...
    return (
        <div className="w-full max-w-[95%] sm:max-w-md mx-auto flex flex-col items-center"> 
            ...
            <header className="px-2">
                <h1 className="text-3xl md:text-5xl font-black text-white text-center md:text-left leading-tight">
                    Accede a tu Cuenta
                </h1>
            ...
            >
                <div className="flex flex-col gap-2">
                    <label className="font-bold text-gray-700 text-lg md:text-xl">Nombre de usuario</label>
                    <input
                        type="text"
                        placeholder="Usuario TracePulse"
                ...
                <div className="flex flex-col gap-2">
                    <label className="font-bold text-gray-700 text-lg md:text-xl">Password</label>
            ...
            <nav className="mt-8 mb-10 flex flex-col space-y-4 px-2">
                <Link to={'/auth/register'} className="text-center text-gray-300 text-sm md:text-base">
                    ¿No tienes cuenta? <span className="text-red-500 font-bold underline">Regístrate</span>
                </Link>
                <Link to={'/auth/forgot-password'} className="text-center text-gray-300 text-sm md:text-base">
                    ¿Olvidaste tu contraseña? <span className="text-red-500 font-bold underline">Click aquí</span>
                </Link>
            </nav>
        </div>
    )
